...
const setupStyles = readFileSync(new URL("../styles/setup.css", import.meta.url), "utf8");
...
describe("setup token migration", () => {
  it("uses typography tokens for setup copy and labels", () => {
    ...
    expect(setupStyles).toMatch(/\.setup-wizard-copy-title\s*{[^}]*font-size:\s*var\(--text-lg\);/s);
    expect(setupStyles).toMatch(/\.setup-wizard-step strong,\s*\.setup-wizard-complete strong\s*{[^}]*font-size:\s*var\(--text-lg\);/s);
    ...
    expect(setupStyles).toMatch(/\.setup-wizard-choice-meta,\s*\.setup-wizard-summary-list\s*{[^}]*font-size:\s*var\(--text-base\);/s);
    expect(setupStyles).toMatch(/\.setup-wizard-choice-default\s*{[^}]*font-size:\s*var\(--text-sm\);/s);
  ...
  it("uses semantic surface tokens for setup stages and cards", () => {
    expect(setupStyles).toMatch(/\.setup-wizard-complete-compact\s*{[^}]*background:\s*var\(--surface-panel-subtle\);/s);
    ...
    expect(setupStyles).toMatch(/\.setup-wizard-body,\s*\.setup-wizard-complete\s*{[^}]*background:\s*var\(--surface-panel-subtle-max\);/s);
    ...
    expect(setupStyles).toMatch(/\.setup-wizard-choice\s*{[^}]*background:\s*var\(--surface-panel-subtle\);/s);
    ...
    expect(setupStyles).toMatch(/\.setup-wizard-choice-default\s*{[^}]*background:\s*var\(--surface-pill-ghost\);/s);
