...
if [[ "$API_TS_PORT" == "8788" ]]; then
  echo "[warn] 8788 is often the packaged Electron API and may require an IPC-only token." >&2
  echo "[warn] Prefer the default dev api-ts port 8799 for standalone Web live e2e." >&2
fi
...
if ! curl -fsS "$API_TS_BASE/api/healthz" >/dev/null 2>&1; then
  ...
  API_TS_PORT="$API_TS_PORT" pnpm --filter @threadlens/api dev >"$ROOT/.web-live-e2e-api-ts.log" 2>&1 &
  TS_PID="$!"
  STARTED_TS=1
fi
...
pnpm --filter @threadlens/web exec playwright install chromium >/dev/null
...
pnpm --filter @threadlens/web exec playwright test -c playwright.live.config.ts --output "$PW_OUTPUT_DIR" 2>&1 | tee "$LOG_FILE"
